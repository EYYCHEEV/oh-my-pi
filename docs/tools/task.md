...
- Batch mode (`task.batch`, default on)
  - on — `{ context, tasks[] }`: one independent spawn per item, required `context` shared across the call's spawns, `agent`/`isolated` per item. Unless `task.batchBlocking` selects the merged multi-item path, lifecycle, revival, and concurrency semantics match N parallel single calls.
