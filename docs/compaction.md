...
- With automatic compaction enabled, the existing pre-send gate checks the prepared request against its captured model and matching tokenizer. The provider-anchored occupancy includes completed output and pending-input growth, while honoring explicit occupancy reports and history rewrites. The gate never sends an oversized request. If provider preparation adds late growth before maintenance has used that boundary's recovery opportunity, the session may compact once through its configured recovery path and rebuild the request. A proactive maintenance attempt consumes the same opportunity. When recovery cannot run, cannot make the request fit, or loses its continuation to cancellation or a newer run, the session retains an actionable refusal instead of blindly resending or reporting a stale answer.
- Successful requests retain a signed prepared-versus-complete-source token difference in their context snapshot. Source messages are measured before transforms, including deferred prompts and steering; unsent source input is charged separately and never becomes formatting credit. Admission charges only formatting growth since a compatible provider/model/tokenizer snapshot, so unchanged tool-schema or transcript-formatting overhead is not charged twice. Older snapshots without this accounting retain the conservative check until a successful request records it; an already-paused legacy session may need one compaction before resuming.
