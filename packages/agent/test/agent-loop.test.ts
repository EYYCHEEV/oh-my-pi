...
describe("agentLoop with AgentMessage", () => {
	...
	it("retries when harmony leakage reaches the committed assistant message (openai-codex)", async () => {
		...
		const config: AgentLoopConfig = { model: codexModel, convertToLlm: identityConverter };
	...
	it("runs completed tool calls after a bun fetch socket-close error", async () => {
		const executedParams: Array<{ value: string }> = [];
		const toolSchema = type({ value: "string" });
		const tool: AgentTool<typeof toolSchema, { value: string }> = {
			name: "echo",
...
describe("agentLoop pre-model-call gate", () => {
	...
	it("closes an open Harmony retry turn when the gate stops the retry", async () => {
		...
		const retryModel = { ...codexModel, id: "retry-model" };
		let gateCalls = 0;
		let turnEndCalls = 0;
		let rejectedToolChoices = 0;
		const config: AgentLoopConfig = {
			model: codexModel,
			getModel: () => (gateCalls === 0 ? codexModel : retryModel),
			convertToLlm: identityConverter,
