...
import { getThemeEpoch, theme, type ThemeColor } from "../theme";
...
/**
 * Structural contract for extension-provided status segments.
 * The extension API owns the concrete definition; the TUI only needs these
 * render-time fields and intentionally avoids a package dependency cycle.
 */
interface ExtensionStatusSegmentDefinition {
	id: string;
	placement: {
		afterBuiltin: string;
...
function removeContextSegments<T>(parts: string[], segments: StatusLineSegmentId[], owners?: T[]): void {
	let writeIndex = 0;
	for (let readIndex = 0; readIndex < segments.length; readIndex++) {
		...
		if (owners) owners[writeIndex] = owners[readIndex];
		writeIndex++;
	}
	parts.length = writeIndex;
	segments.length = writeIndex;
	if (owners) owners.length = writeIndex;
}
...
export class StatusLineComponent<TSession extends StatusLineSession = StatusLineSession> implements Component {
	...
	#renderStatusLine(
		...
		for (const definition of this.#extensionSegments.values()) {
			...
			if (leftAnchor >= 0) {
				if (layout === "plain-right") continue;
		...
		if (embedContext) {
			removeContextSegments(leftParts, leftSegIds, leftPartOwners);
			removeContextSegments(rightParts, rightSegIds, rightPartOwners);
		}
