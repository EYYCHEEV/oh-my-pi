...
export class SessionMaintenance {
	...
	claimPreparedBudgetRecovery(): PreparedBudgetRecoveryOwner | undefined {
		...
		if (
			...
			(!this.#usesExperimentalContextManagement() && !hasConfiguredCompactionMethod(settings))
		) {
			return undefined;
		}
	...
	async #runExperimentalContextRollover(
		...
		try {
			...
			if (!safeToContinue) {
				...
				if (!options.suppressDeadEndNotice) this.#host.emitNotice("warning", warning, "compaction");
	...
	async maintainContextMidRun(
		...
		const crossedThresholdSinceProviderCall =
			providerReportedContextTokens < resolveThresholdTokens(contextWindow, compactionSettings);
		if (!shouldCompact(contextTokens, contextWindow, compactionSettings)) {
			this.maybeStartSpeculativeCompaction(contextTokens, contextWindow);
			return;
		}
		...
		if (
			!crossedThresholdSinceProviderCall &&
