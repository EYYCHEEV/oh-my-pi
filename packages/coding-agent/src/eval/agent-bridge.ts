...
export async function runEvalAgent(args: unknown, options: EvalAgentBridgeOptions): Promise<EvalAgentResult> {
	...
	const baseRunOptions: ExecutorOptions = {
		...
		preloadedExtensionPaths: options.session.extensionPaths,
		requiredExtension: options.session.requiredExtension,
		preloadedCustomToolPaths: options.session.customToolPaths,
