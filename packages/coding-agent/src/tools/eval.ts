...
import { sessionRequiresDelegation } from "../task/prompt-policy";
...
export interface EvalToolDescriptionOptions {
	...
	evalTools?: boolean;
	/** Push `workpool()` as the default only under explicit `task.eager: always`. Default: false. */
...
export function getEvalToolDescription(options: EvalToolDescriptionOptions = {}): string {
	...
	return prompt.render(evalDescription, {
		...
		eagerDelegation: options.eagerDelegation ?? false,
	...
	get description(): string {
		...
		if (!this.session) {
			...
			base = getEvalToolDescription({
				...
				eagerDelegation: sessionRequiresDelegation(this.session),
