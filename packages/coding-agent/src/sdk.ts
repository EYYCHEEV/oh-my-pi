...
export async function createAgentSession(options: CreateAgentSessionOptions = {}): Promise<CreateAgentSessionResult> {
	...
	try {
		...
		if (restrictToolNames) {
			// Allocate an empty session runtime without evaluating caller-provided
			// extension instances, paths, or factories.
			extensionPaths = [];
		...
		for (const factory of restrictToolNames ? [] : (options.extensions ?? [])) {
