...
export function createAcpSessionFactory(args: AcpSessionFactoryOptions): AcpSessionFactory {
	return async (cwd, factoryOptions) => {
		...
		let setToolUIContext: CreateAgentSessionResult["setToolUIContext"];
		try {
			...
			({ session: nextSession, setToolUIContext } = await args.createSession({
				...args.baseOptions,
				cwd,
...
export async function buildSessionOptions(
	...
	const options: CreateAgentSessionOptions = {
		...
		requiredRuntimeExtensions: activeSettings.getHost("requiredRuntimeExtensions"),
	};
	...
	try {
		...
		if (mode === "acp") {
			...
			if (mode === "rpc" || mode === "rpc-ui") {
				...
				let exitCode: number;
				try {
					exitCode = await runPrintMode(session, {
						mode,
