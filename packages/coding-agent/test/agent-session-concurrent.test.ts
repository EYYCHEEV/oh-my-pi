...
describe("AgentSession concurrent prompt guard", () => {
	...
	async function createAbortableLifecycleSession(extensionRunner?: ExtensionRunner): Promise<void> {
			...
			streamFn: (_model, _context, options) => {
				...
				queueMicrotask(() => {
					...
					options?.signal?.addEventListener(
						"abort",
						() => {
							stream.push({ type: "error", reason: "aborted", error: makeMsg("Aborted", "aborted") });
						},
						{ once: true },
					);
				});
	...
	it("settles an Esc-aborted root run as interrupted", async () => {
		await createAbortableLifecycleSession();
		...
		session.subscribe(event => {
			...
			if (event.state === "running") running.resolve();
		});
	...
	it("does not emit a settled-only lifecycle when pre-agent setup fails", async () => {
		...
		await createAbortableLifecycleSession(extensionRunner);
	...
	it("keeps an aborted pre-agent finalizer from settling the next prompt", async () => {
		...
		const firstSetupEntered = Promise.withResolvers<void>();
		let setupCount = 0;
		const extensionRunner = {
			emit: vi.fn().mockResolvedValue(undefined),
			emitBeforeAgentStart: vi.fn(() => {
				setupCount++;
				if (setupCount !== 1) return Promise.resolve();
		...
		const nextRunning = Promise.withResolvers<string>();
		session.subscribe(event => {
			if (event.type === "agent_activity_state" && event.state === "running") nextRunning.resolve(event.runId);
		});

		const nextPrompt = session.prompt("next run");
		const nextRunId = await nextRunning.promise;
		await session.abort({ reason: USER_INTERRUPT_LABEL });
...
describe("AgentSession TTSR resume gate", () => {
	...
	it("prompt() waits for context-promotion continuation to finish", async () => {
		...
		// so pin gpt-5.5 (272k) -> gpt-5.6-luna (372k) via modelOverrides.
		...
		await Bun.write(
			...
			JSON.stringify({
				providers: {
					"openai-codex": {
						modelOverrides: {
							"gpt-5.5": { contextPromotionTarget: "openai-codex/gpt-5.6-luna" },
						},
					},
				},
			}),
		...
		const largeModel = modelRegistry.find("openai-codex", "gpt-5.6-luna");
