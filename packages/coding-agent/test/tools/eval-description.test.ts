...
describe("eval tool description", () => {
	...
	it("uses forceful delegation wording only for task.eager always", () => {
		const defaultTool = new EvalTool(makeSession({ spawns: "*", taskEager: "default" }));
		const preferredTool = new EvalTool(makeSession({ spawns: "*", taskEager: "preferred" }));
		const alwaysTool = new EvalTool(makeSession({ spawns: "*", taskEager: "always" }));

		expect(defaultTool.docTopics().agents).toContain("Keep-alive worker pool");
		expect(preferredTool.docTopics().agents).toContain("Keep-alive worker pool");
		expect(defaultTool.docTopics().agents).not.toContain("Default for 2+ independent items");
		expect(preferredTool.docTopics().agents).not.toContain("Default for 2+ independent items");
		expect(alwaysTool.docTopics().agents).toContain("Default for 2+ independent items");
		expect(defaultTool.description).not.toContain("For 2+ independent items");
		expect(preferredTool.description).not.toContain("For 2+ independent items");
		expect(alwaysTool.description).toContain("For 2+ independent items");
	});
