...
describe("queued user delivery policy", () => {
	...
	function setup(
		...
		return {
			...
			delivered,
			manager,
			// The runner owns a fixed identity snapshot; register more handlers on a known fixture.
			addBeforeStartHandlers: (...additional: Extension[]) => {
				const handlers = extensions[1].handlers.get("before_agent_start")!;
				for (const extra of additional) handlers.push(...(extra.handlers.get("before_agent_start") ?? []));
			},
	...
	it.each(["ordinary", "queued"] as const)(
		...
		async delivery => {
			let recalls = 0;
			const { requests, delivered, addBeforeStartHandlers, pausePreparation } = await setupMemory(
				"hindsight",
				async () => `override-memory-${++recalls}`,
				true,
			);
			let attempts = 0;
			addBeforeStartHandlers(
	...
	it("preserves absolute overrides and their downstream chain after a source-base retry", async () => {
		const { requests, addBeforeStartHandlers, pausePreparation } = await setupMemory(
			"hindsight",
			async () => "hidden memory",
			true,
		);
	...
	it("retries a base change during returned-image normalization before publishing the request", async () => {
		const { requests, addBeforeStartHandlers } = await setupMemory("hindsight", async () => "image memory", true);
		const image: ImageContent = {
			type: "image",
			mimeType: "image/png",
			data: "iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mP8/x8AAwMCAO+jD3sAAAAASUVORK5CYII=",
	...
	it("pauses automatic draining after live queued policy exhaustion until an explicit retry", async () => {
		...
		const { agent, requests, delivered, addBeforeStartHandlers } = setup(
			[
